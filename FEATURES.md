...
- **Track info page** (`public/js/track-info.js`) — tapping the now-playing bar a second time, while already on the Now Playing screen, opens a full-screen page with every embedded tag the parsers found (title/artist/album/album artist/genre/year/track/composer) plus file info (name, folder, format, size, duration) and artwork, skipping any field that isn't present on that particular file. Read-only, closed with an explicit Close button.
