...
- **YouTube → Dropbox importer** (`downloader/`) — a separate Python service (not part of the Cloudflare Pages/Worker app, since it needs to run real `yt-dlp`/`ffmpeg`/`mutagen` processes): paste a YouTube video or playlist link, edit title/artist/album/genre/track/year and cover art per track or in bulk, then save the tagged MP3s (or original MP4 for non-music links) into a `YouTube Downloads/` Dropbox inbox via `rclone`. See `downloader/README.md` for setup/deploy.
